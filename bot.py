...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если пользователь НЕ в базе - даем подсказку
    if not user_info:
        await update.message.reply_text(
            f"❌ Кодовое слово введено неверно.\n\n"
            f"🔑 Для получения материалов введите правильное кодовое слово.\n\n"
            f"Используйте /help для получения дополнительной информации."
        )
        return
